...
public class CameraPoseEstimator : MonoBehaviour {
    ...
    private void GetCameraPose(Mat rvec, Mat tvec)
    {
        Mat _tvec = new Mat(3, 1, tvec.type());

        //Debug.Log("rvec " + tvec.get(0, 0)[0] + " " + tvec.get(0, 0)[1] + " " + tvec.get(0, 0)[2]);

        _tvec.put(0, 0, new double[] { tvec.get(0, 0)[0], 0, 0 });
        _tvec.put(1, 0, new double[] { tvec.get(0, 0)[1], 0, 0 });
        _tvec.put(2, 0, new double[] { tvec.get(0, 0)[2], 0, 0 });

        Mat _rvec = new Mat(3, 1, tvec.type());
        ...
        //Debug.Log(cameraR.cols());
        //Debug.Log(cameraR.rows());
        _tvec = -cameraR * _tvec;

        //Debug.Log("tvec " + _tvec.get(0, 0)[0] + " " + _tvec.get(1, 0)[0] + " " + _tvec.get(2, 0)[0]);
        //Debug.Log("rvec " + cameraRotation.get(0, 0)[0] + " " + cameraRotation.get(1, 0)[0] + " " + cameraRotation.get(2, 0)[0]);

        PoseData poseData = ARUtils.ConvertRvecTvecToPoseData(rvec.get(0, 0), tvec.get(0, 0));

        //Debug.Log(poseData.rot.w + " " + poseData.rot.x + " " + poseData.rot.y+ " " + poseData.rot.z);
        //Debug.Log("posedata: " + poseData.pos.x + " " + poseData.pos.y + " " + poseData.pos.z + " ");
        ARM = ARUtils.ConvertPoseDataToMatrix(ref poseData, true, true);
